...
<div class="container_box">
  ...
  <div id="box">
      ...
      <div class="header">
        ...
        <h3>เพิ่มข้อมูลโปรโมชั่นของแถม</h3>
...
<form  method="post" name="frmuser" id="frmuser" action="?page=save_giveaway" >
...
<tr>
<td width="300"><input type="checkbox" id="checkItem_<?=$P_Code;?>" name="checkItem_<?=$P_Code;?>" value="<?=$P_Code;?>" /><?=$open1['PRODUCTNAME'];?></td>

	<td width="400">จำนวนเริ่มต้นที่&nbsp;&nbsp;<input type="text" id="count_<?=$P_Code;?>" name="count_<?=$P_Code;?>" size="10" disabled="" onKeyUp="if(this.value*1!=this.value) this.value='' ;" >&nbsp;<?=$open1['st_unit_id'];?></td>
	<td width="300">ได้ของแถม
	<select id="free_<?=$P_Code;?>" name="free_<?=$P_Code;?>"  style="width:150px;" required disabled>
	<option value="">-เลือก-</option>
	<?	$sql="SELECT * from st_item_product ";
			...
			?>
			<option value="<?print $R=$detail['P_Code']?>" ><?print $detail['PRODUCTNAME']?></option>
		<?}?>

	</select></td>
	<td width="400">จำนวน&nbsp;&nbsp;<input type="text" id="freecount_<?=$P_Code;?>" name="freecount_<?=$P_Code;?>" size="10" disabled="" onKeyUp="if(this.value*1!=this.value) this.value='' ;">
	&nbsp;หน่วย</td>
</tr>
