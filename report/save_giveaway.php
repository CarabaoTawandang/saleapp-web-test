...
for($j=0;$j<$row;$j+=1) 
			{	
				...
				$free="free_".$P_Code;//รหัสสินค้าของแถม
				$freecount ="freecount_".$P_Code;//จำนวน
				
			$item=$_POST[$checkItem];//ชื่อสินค้าที่มีการติก
			$count_=$_POST[$count];//จำนวนที่มีการติก
			$free_=$_POST[$free];//รหัสสินค้า
			$freecount_=$_POST[$freecount];//จำนวน
...
(PromotionId,P_Code_start,TypeList,PRODUCTNAME_start
,Createby,Updateby,CreateDate,UpdateDate,COMPANYCODE,Updatestatus,Condition,Parameter,P_Code_end,Qty_P_Code_end,QtyChk) values
('$ID_PRO','$item','0','$op2[PRODUCTNAME]'
,'$USER_id','$USER_id',GETDATE(),GETDATE(),'$txt_COMPANY','1','>=','*','$free_','$freecount_','$count_') ";
...
,GETDATE(),GETDATE(),'$txt_COMPANY','1','002','$DC','ของแถม') ";
