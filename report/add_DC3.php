...
$txt_COMPANY =$_POST['txt_COMPANY'];
		$sqlMax="select SUBSTRING(max(dc_groupid),5,5) as MaxDC from st_user_group_dc where   SUBSTRING(dc_groupid,3,2) ='$year'";
		$params = array();
...
$CodeIdShow ="DC".$year.str_pad($CodeId,5,"0",STR_PAD_LEFT);
				...
				$sqlSave1="insert into st_user_group_dc (dc_groupid,dc_groupname,Createby ,Updateby,CreateDate,UpdateDate,Updatestatus,COMPANYCODE) 
				values ('$CodeIdShow','$txtNameDC','$USER_id','$USER_id',GETDATE(),GETDATE(),'1','$txt_COMPANY')";
	...
	{  //echo "<br> อ. ".$DetailAmp;
		...
		if($reAmp['AMPHUR_CODE'])
			{	//echo "<br> ภ.  ".$reAmp['GEO_ID']; echo "  จ.  ".
				...
				('$CodeIdShow','$reAmp[GEO_ID]','$reAmp22[PROVINCE_CODE]','$reAmp[AMPHUR_CODE]','1')";
