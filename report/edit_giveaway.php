...
<div class="container_box">
    <div id="box">
	<div class="header"><h3>แก้ไขโปรโมชั่นของแถม&nbsp;<?=$pp['PromotionName'];?></h3>
           <p>&nbsp;</p><!---หัวเรื่องรอง-->
		...
		<!---เนื้อหา-->
<form  method="post" name="frmuser" id="frmuser" action="?page=save_giveaway&do=edit" >
...
$(function(){
		...
		$("#freecount_<?=$P_Code;?>").prop("disabled", !$(this).is(':checked'));
					if(document.frmuser.freecount_<?=$P_Code;?>.disabled)
					{document.frmuser.freecount_<?=$P_Code;?>.value = "";
					}
					else {document.frmuser.freecount_<?=$P_Code;?>.value = "";}
					
		$("#free_<?=$P_Code;?>").prop("disabled", !$(this).is(':checked'));
					if(document.frmuser.free_<?=$P_Code;?>.disabled)
					{document.frmuser.free_<?=$P_Code;?>.value = "";
					}
					else {document.frmuser.free_<?=$P_Code;?>.value = "";}
...
<tr><?
	...
	?>
<td width="300"><input type="checkbox" id="checkItem_<?=$P_Code;?>" name="checkItem_<?=$P_Code;?>" value="<?=$P_Code;?>" <? if($reCheck['P_Code_start']){echo "checked";}?>/>
<?=$open1['PRODUCTNAME'];?></td>

	<td width="400">จำนวนเริ่มต้นที่&nbsp;&nbsp;<input type="text" id="count_<?=$P_Code;?>" name="count_<?=$P_Code;?>" value="<?=$reCheck['QtyChk'];?>" size="10" onKeyUp="if(this.value*1!=this.value) this.value='' ;" <? if(!$reCheck['P_Code_start']){echo'disabled=""';} ?>>&nbsp;<?=$open1['st_unit_id'];?></td>
	<td width="300">ได้ของแถม
	<select id="free_<?=$P_Code;?>" name="free_<?=$P_Code;?>"  style="width:150px;" <? if(!$reCheck['P_Code_start']){echo'disabled=""';} ?> required >
	<? if(!$reCheck['P_Code_start']){?>
	<option value="">-เลือก-</option><?}
	else{?>
	<option value="<?=$reCheck['P_Code_end'];?>">
	<?
	$name="SELECT * from st_item_product where P_Code ='$reCheck[P_Code_end]' ";
	$name=sqlsrv_query($con,$name,$params,$options);
	$name=sqlsrv_fetch_array($name);
	echo $name['PRODUCTNAME'];
	?></option>
	<?}	$sql="SELECT * from st_item_product ";
	...
	<td width="400">จำนวนของแถม&nbsp;&nbsp;<input type="text" id="freecount_<?=$P_Code;?>" name="freecount_<?=$P_Code;?>" value="<?=$reCheck['Qty_P_Code_end'];?>" <? if(!$reCheck['P_Code_start']){echo'disabled=""';} ?> size="10"  onKeyUp="if(this.value*1!=this.value) this.value='' ;">&nbsp;หน่วย</td>
</tr>
