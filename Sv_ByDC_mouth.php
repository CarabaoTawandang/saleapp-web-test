<?//------------------------------------------------------------------web นี้สร้างโดย Numphung(น้ำผึ้ง) ปี2557
	...
	header("Content-Disposition: attachment;filename=SV_ByMonht".date('Ymd').".xls"); 
...
<div align="center">
ข้อมูลสำรวจเฉพาะเดือน 12-2015 <? $YM="2015-12";?>
<br>เครื่องหมาย- หมายถึง:ไม่มีการกระทำใดๆ
	...
	<? 
$SqlTotal="select top 2  u.dc_groupid,dc.dc_groupname 
...
where cast(w.CreateDate as date ) like '$YM%' 
...
where cast(w.CreateDate as date ) like '$YM%' and  Md.QuestionType like 'อื่นๆ%' and u.dc_groupid='$reTotal[dc_groupid]'
...
<? } ?>
	<tr >
	<td colspan="2"  bgcolor="#FFFFCC" >รวม</td>
	<td bgcolor="#FFFFCC"><?=number_format($Total);?></td>
	</tr>
	</table><br><br>
...
from st_Master_Survey_answer w left join st_user u on w.Createby = u.User_id left join st_user_group_dc dc on u.dc_groupid = dc.dc_groupid 
left join st_Master_Survey_detail Md on w.SurveyID = Md.SurveyID and w.Row= Md.Row
where cast(w.CreateDate as date ) like '$YM%'  and w.Answer !='-' 
...
?>
	...
	<td><?if($Question != $reTotalByDcByQuestion['Question'])
	{ echo $reTotalByDcByQuestion['Row']."  ".$reTotalByDcByQuestion['Question']; $aa=1;} 
	$Question = $reTotalByDcByQuestion['Question'];
	?>
	</td>
	<td><? echo $reTotalByDcByQuestion['Answer'];?></td>
	
	<td><? echo number_format($reTotalByDcByQuestion['TotalByDcByQuestion']); 
	if($Question == $reTotalByDcByQuestion['Question'])
	{$TotalAnswer=$TotalAnswer+$reTotalByDcByQuestion['TotalByDcByQuestion'];}?></td>
	</tr>
...
//------------รวม
if($aa>1)
{echo '<tr><td colspan="3" bgcolor="#FFFFCC" align="right">รวม</td><td bgcolor="#FFFFCC"><b>'.number_format($TotalAnswer)  .'</b></td></tr>'; 
$TotalAnswer='';
}
...
$dc_groupidON = $reTotalByDcByQuestion['dc_groupid']; $aa++; 
} ?>
<?
	...
	{			$sql3="select PRODUCTNAME from st_item_product where P_Code ='$expArrays'";
				...
				$t=1;
			for($i=1;$i<=2;$i++){ if($i==1){$Type22='มี';}if($i==2){$Type22='ไม่มี';} //if($i==3){$Type22='-';} 
				echo '<tr><td></td>';
				...
				where   w.Answer like '%$expArrays,$Type22%' and u.dc_groupid='$dc_groupids' and cast(w.CreateDate as date ) like '$YM%'  ";
				$sqlQty=sqlsrv_query($con,$sqlQty);
				$sqlQty=sqlsrv_fetch_array($sqlQty);
				echo '<td>'.number_format($sqlQty['total3']).'</td></tr>'; $Total_total3=$Total_total3+$sqlQty['total3'];
				$t++;
				//------------รวม
				if($t>2)
				{echo '<tr><td colspan="3" bgcolor="#FFFFCC" align="right">รวม</td><td bgcolor="#FFFFCC"><b>'.number_format($Total_total3)  .'</b></td></tr>'; 
				$Total_total3='';
				}
			}
	}
